...
class update_matrices_tests(unittest.TestCase):
    ...
    def test_solve(self):
        ...
        nptest.assert_allclose(res.x,
            np.array([0.85459329, 0.73472366, 0.06156, -0.06095794, -0.96167612]), rtol=1e-5, atol=1e-5)
        nptest.assert_allclose(res.y,
            np.array([0., 0., 0., -2.32275314, 0., -0.93213354, 0., -0.8939565]), rtol=1e-5, atol=1e-5)
        nptest.assert_allclose(res.info.obj_val, -1.5116431929127323, rtol=1e-5, atol=1e-5)
    ...
    def test_update_P(self):
        ...
        # Assert close
        nptest.assert_allclose(res.x,
            np.array([0.79105808, 0.68008954, -0.00974931, -0.00569589, -0.92142316]), rtol=1e-5, atol=1e-5)
        nptest.assert_allclose(res.y,
            np.array([0., -0.0977256, 0., -2.3051196, 0., -0.84705904, 0., -0.9014214]), rtol=1e-5, atol=1e-5)
        nptest.assert_allclose(res.info.obj_val, -1.40901946656505, rtol=1e-5, atol=1e-5)
    ...
    def test_update_P_A_indP_indA(self):
        ...
        # Assert close
        nptest.assert_allclose(res.x,
            np.array([0.45599336, 0.11471169, 0.22567378, -0.80654725, -0.01778191]), rtol=1e-5, atol=1e-5)
        nptest.assert_allclose(res.y,
            np.array([-1.76495387, -1.44638239, 0., 0., -1.28476339, 0., 0., 0.]), rtol=1e-5, atol=1e-5)
        nptest.assert_allclose(res.info.obj_val, -0.8249598023368026, rtol=1e-5, atol=1e-5)
